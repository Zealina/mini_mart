    ...
    <div className="min-h-screen bg-[#f1f1f2] font-sans text-[#282828] flex flex-col relative">
      ...
      <nav className="bg-white shadow-sm p-3 flex justify-between items-center px-4 sm:px-8 sticky top-0 z-40">
        <Link to="/" className="flex items-center">
          <img src="/logo-vertical.png" alt="CEXPRESS MINIMART" className="h-10 md:h-12 object-contain mix-blend-multiply" />
        </Link>
          ...
          {actualUser ? (
            <div className="relative">
              ...
              >
                ...
                <span className="hidden sm:block text-sm">Hi, {actualUser.first_name || 'User'}</span>
      ...
      <div className="md:hidden px-4 py-3 bg-white border-b border-gray-100">
        <div className="relative">
          <input 
            ...
            className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#f68b1e] text-sm"
          />
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 w-full">
        ...
        {/* DYNAMIC HERO SLIDER */}
        <div className="relative w-full h-[180px] sm:h-[250px] md:h-[400px] rounded-2xl overflow-hidden shadow-md mb-8 group bg-white">
          ...
          <button 
            ...
            className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 z-20 bg-black/20 hover:bg-black/40 text-white p-1.5 md:p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-sm"
          >
            <ChevronLeft className="h-5 w-5 md:h-6 md:w-6" />
          </button>
          <button 
            onClick={() => setCurrentSlide(prev => prev === slides.length - 1 ? 0 : prev + 1)} 
            className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 z-20 bg-black/20 hover:bg-black/40 text-white p-1.5 md:p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-sm"
          >
        ...
        <div className="flex flex-col lg:flex-row gap-6 md:gap-8">
          <div className="w-full lg:w-64 flex-shrink-0">
            <div className="bg-white rounded-xl p-4 md:p-5 border border-gray-100 shadow-sm lg:sticky lg:top-24">
          ...
          <div className="flex-grow">
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-gray-100 pb-4">
              ...
              <span className="text-xs md:text-sm text-gray-500 font-medium">{filteredProducts.length} Items found</span>
            </div>
            
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border border-gray-100 p-12 text-center shadow-sm">
                <ShoppingBag className="h-12 w-12 text-gray-300 mx-auto stroke-1 mb-4" />
                <h3 className="text-base md:text-lg font-bold text-gray-700">No Products Found</h3>
              </div>
            ) : (
                  ...
                  >
                    <div className="relative h-32 md:h-48 overflow-hidden bg-gray-50 flex items-center justify-center">
                      {product.image_url ? (
                        ...
                        <ShoppingBag className="h-8 w-8 md:h-12 md:w-12 text-gray-300 stroke-1" />
                      )}
                    </div>
                    <div className="p-3 md:p-4 flex flex-col flex-grow">
                      <h3 className="text-xs md:text-sm font-semibold text-gray-800 line-clamp-2 h-8 md:h-10 leading-tight group-hover:text-[#f68b1e] transition-colors">{product.name}</h3>
                      <p className="text-[10px] md:text-xs text-gray-400 mt-1 truncate">{product.brand || 'Generic'}</p>
                      <div className="mt-auto pt-3 md:pt-4 border-t border-gray-50 flex flex-col">
                        <span className="text-sm md:text-lg font-bold text-gray-900">₦{parseFloat(product.price).toLocaleString()}</span>
                        {product.stock <= 0 ? (
                          <span className="text-[10px] md:text-xs font-semibold text-red-500 mt-2 text-center bg-red-50 py-1.5 rounded-md md:rounded-lg">Out Of Stock</span>
                        ) : (
                          <button 
                            onClick={(e) => { e.stopPropagation(); addToCart(product); }}
                            className="w-full mt-2 md:mt-3 bg-white border border-[#f68b1e] text-[#f68b1e] py-1.5 md:py-2 rounded-md md:rounded-lg text-xs md:text-sm font-bold hover:bg-[#f68b1e] hover:text-white transition-colors shadow-sm flex justify-center items-center gap-1.5 md:gap-2"
                          >
                            <ShoppingCart className="h-3.5 w-3.5 md:h-4 md:w-4" /> <span className="hidden sm:inline">ADD TO CART</span><span className="sm:hidden">ADD</span>
                          </button>
                        )}
                      </div>
                    </div>
      ...
      <footer className="bg-[#282828] text-white py-12 mt-auto">
        <div className="max-w-7xl mx-auto px-4 flex flex-col items-center text-center">
          <img src="/logo-circular.png" alt="CEXPRESS MINIMART" className="h-16 w-16 md:h-20 md:w-20 rounded-full mb-4 shadow-lg object-contain bg-white" />
          <p className="text-gray-400 text-xs md:text-sm">© 2026 CEXPRESS MINIMART. All Rights Reserved.</p>
        </div>
      </footer>
      ...
      {viewProduct && (
        ...
        >
          ...
          >
            <button 
              onClick={() => setViewProduct(null)}
              className="absolute top-2 right-2 md:top-4 md:right-4 bg-white/80 p-2 rounded-full text-gray-500 hover:text-red-500 hover:bg-red-50 z-10 transition-colors shadow-sm backdrop-blur-md"
            >
              <X className="h-5 w-5" />
            </button>
            
            <div className="w-full md:w-1/2 bg-gray-50 min-h-[250px] md:min-h-full flex items-center justify-center p-4 md:p-6 border-b md:border-b-0 md:border-r border-gray-100">
              {viewProduct.image_url ? (
                <img src={viewProduct.image_url} alt={viewProduct.name} className="w-full h-auto max-h-[300px] object-contain drop-shadow-md rounded-xl" />
              ) : (
                <ShoppingBag className="h-16 w-16 md:h-24 md:w-24 text-gray-300 stroke-1" />
              )}
            </div>

            <div className="w-full md:w-1/2 p-5 md:p-8 flex flex-col">
              <span className="text-[10px] md:text-xs font-bold text-[#f68b1e] uppercase tracking-wider mb-1 md:mb-2">
                {categories.find(c => c.id === viewProduct.category_id)?.name || 'Product Detail'}
              </span>
              <h2 className="text-xl md:text-2xl font-black text-gray-900 leading-tight mb-2">{viewProduct.name}</h2>
              <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-4 md:mb-6 border-b border-gray-100 pb-3 md:pb-4">
                <span className="flex items-center gap-1 text-xs md:text-sm font-semibold text-gray-600 bg-gray-100 px-2 md:px-2.5 py-1 rounded-md">
                  <Tag className="h-3 md:h-3.5 w-3 md:w-3.5" /> {viewProduct.brand || 'No Brand'}
                </span>
                {viewProduct.package_size && (
                  <span className="text-xs md:text-sm font-semibold text-gray-600 bg-gray-100 px-2 md:px-2.5 py-1 rounded-md">
                    {viewProduct.package_size}
                  </span>
                )}
              </div>
              ...
              <div className="flex-grow">
                <h4 className="text-xs md:text-sm font-bold text-gray-900 mb-1.5 md:mb-2">Description</h4>
                <p className="text-gray-500 text-xs md:text-sm leading-relaxed mb-4 md:mb-6 whitespace-pre-wrap">
              ...
              <div className="mt-auto pt-4 md:pt-6 border-t border-gray-100">
                <div className="flex justify-between items-end mb-4">
                  <div>
                    <span className="block text-[10px] md:text-xs text-gray-400 font-bold uppercase mb-0.5 md:mb-1">Price</span>
                    <span className="text-2xl md:text-3xl font-black text-gray-900">₦{parseFloat(viewProduct.price).toLocaleString()}</span>
                  </div>
                  <span className={`px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-bold ${viewProduct.stock > 0 ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                ...
                <button 
                  ...
                  className={`w-full py-3 md:py-3.5 rounded-xl text-sm md:text-base font-bold flex justify-center items-center gap-2 transition-all shadow-md ${viewProduct.stock > 0 ? 'bg-[#f68b1e] hover:bg-orange-600 text-white' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
                >
